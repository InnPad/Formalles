...
Ext.define('Ext.ux.FolderTree', {
    ...
    initComponent: function () {
        ...
        this.listeners = {
            'selectionchange': function (sm) {
                ...
                if (this.selectionChange !== undefined && this.selectionChange !== null && this.selectionChange !== Ext.emptyFn) {

                    this.selectionChange(path);
                }
            }
        };

        var folderStore = Ext.create('Ext.data.TreeStore', {
            model: 'Ext.ux.FolderModel',
            autoSync: true,
            nodeParam: 'id',
            root: {
                text: 'Root',
            ...
            listeners: {
                // Each Ext.ux.FolderModel instance will be automatically   
                // decorated with methods/properties of Ext.data.NodeInterface   
                // (i.e., a "node"). Whenever a FolderModel node is appended  
                ...
                append: function (thisNode, newChildNode, index, eOpts) {
                    ...
                    if (!newChildNode.isRoot()) {
                        ...
                        // All folders can be expandable in the tree  
                        newChildNode.set('leaf', false);

                        // Use the model's "Id" value as the id for each tree item  
        ...
        // Graft our folderStore into the settingsTreeStore. Note that the call  
        // to .expand() is what triggers the userTreeStore to load its data.  
        //settingsTreeStore.getRootNode().appendChild(folderStore.getRootNode()).expand();

        this.store = folderStore;
        folderStore.getRootNode().expand();

        this.dropConfig = {
            ddGroup: 'fileMoveDD',
            appendOnly: true,
            onNodeDrop: Ext.Function.bind(this.onTreeNodeDrop, this)
        };

        /*this.selModel = new Ext.tree.DefaultSelectionModel({
            listeners: {
                selectionchange: Ext.Function.bind(this.onTreeSelectionChange, this)
            }
        });*/
        ...
        /*this.listeners = {
            contextmenu: Ext.Function.bind(this.onTreeContextMenu, this),
            afterrender: {
        ...
        /*this.treeEditor = new Ext.ux.tree.TreeEditing(this, {
            allowBlank: false,
            grow: true,
            growMin: 90,
            growMax: 240
            ...
            selectOnFocus: true,
            listeners: {
                beforestartedit: Ext.Function.bind(this.onTreeEditorBeforeStartEdit, this),
                beforecomplete: Ext.Function.bind(this.onTreeEditorBeforeComplete, this)
            }
        });*/
    ...
    *
    * @param	{Ext.ux.tree.TreeEditing}	editor	The tree editor
    ...
    *
    * @private
    * @param	{Ext.ux.tree.TreeEditing}	editor		The tree editor where the node was edited
    ...
    * @returns	{Void}
    */
    onTreeLoaderBeforeLoad: function (store, operation, eOpts) {
        operation.params.action = 'LoadFolder';
        operation.params.path = operation.node.getPath('text');
    ...
    */
    ...
    /**
    * Gets the full path of the current selected folder
    *
    * @returns {String} The full path of the current selected folder
    */
